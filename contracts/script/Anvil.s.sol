...
contract GridTradeHookScript is Script {
    ...
    function run() public {
        ...
        (address hookAddress, bytes32 salt) =
            HookMiner.find(CREATE2_DEPLOYER, permissions, type(GridTradeHook).creationCode, abi.encode(address(manager), ""));

        // ----------------------------- //
        // Deploy the hook using CREATE2 //
        // ----------------------------- //
        vm.broadcast();
        GridTradeHook hook = new GridTradeHook{salt: salt}(IPoolManager(address(manager)), "");
        require(address(hook) == hookAddress, "GridTradeHookScript: hook address mismatch");
        ...
        vm.startBroadcast();
        testLifecycle(address(hook), initializeRouter, lpRouter, swapRouter);
        vm.stopBroadcast();
    }
